...
export default function Home() {
  ...
  // 处理搜索
  const handleSearch = useCallback(async (titles: string) => {
    // 处理空搜索情况
    if (!titles.trim()) {
      setSearchTitles("")
      setMovies([])
    ...
    try {
      const response = await fetch(`/api/movies?titles=${encodedTitles}`)
    ...
    <div className="min-h-screen bg-gradient-to-b from-netflix-black to-black text-white">
      <div className="container mx-auto px-4 py-12">
        <div className="flex flex-col items-center mb-12">
          ...
          <h1 className="text-4xl md:text-6xl font-bold text-center mb-2">拼好影</h1>
        ...
        <div className="max-w-7xl mx-auto">
          <MovieSearch onSearch={handleSearch} initialValue={searchTitles} />
          <MovieResults movies={movies} loading={isLoading} error={error} hasSearched={hasSearched} />
        </div>
      </div>
    </div>
  )
